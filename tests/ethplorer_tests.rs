#[cfg(test)]
mod tests {
    use ethplorer::{api_key_param, GET_ADDRESS_HISTORY, get_address_history_config, get_address_info_config, GET_ADDRESS_INFO_ROUTE, get_address_transactions_config, GET_ADDRESS_TRANSACTIONS_ROUTE, get_last_block_config, GET_LAST_BLOCK_ROUTE, get_token_daily_price_history_config, get_token_daily_transaction_count_config, GET_TOKEN_DAILY_TRANSACTION_COUNT_ROUTE, get_token_history_config, GET_TOKEN_HISTORY_ROUTE, get_token_info_config, GET_TOKEN_INFO_ROUTE, GET_TOKEN_PRICE_HISTORY_GROUPED_ROUTE, get_tokens_new_config, GET_TOKENS_NEW_ROUTE, get_top_config, GET_TOP_ROUTE, get_top_token_holders_config, GET_TOP_TOKEN_HOLDERS_ROUTE, get_top_tokens_config, GET_TOP_TOKENS_ROUTE, GetAddressHistoryParams, GetAddressInfoParams, GetAddressTransactionsParams, GetTokenHistoryParams, GetTopParams, NETWORK};
    ...
    fn get_token_info_config_works() {
        ...
        assert_eq!(config.params, vec![("apiKey".to_string(), "freekey".to_string())]);
    ...
    fn get_token_daily_transaction_count_config_works() {
        ...
        assert_eq!(config.network, NETWORK);
        assert_eq!(config.routes, vec![GET_TOKEN_DAILY_TRANSACTION_COUNT_ROUTE, "0x0"]);
        assert_eq!(config.params, vec![
            ("apiKey".to_string(), "freekey".to_string()),
            ("period".to_string(), "50".to_string()),
        ]);
        assert_eq!(config.to_string(), "https://api.ethplorer.io/getTokenHistoryGrouped/0x0");
    }

    #[test]
    fn get_token_history_config_works() {
        let config = get_token_history_config("", "0x0", &GetTokenHistoryParams {
        ...
        assert_eq!(config.to_string(), "https://api.ethplorer.io/getTokenHistory/0x0");
    }

    #[test]
    fn get_address_history_config_works() {
        let config = get_address_history_config("", "0x0", &GetAddressHistoryParams {
            history_type: "type".to_string(),
            limit: 50,
            timestamp: Default::default(),
            token: "token".to_string()
        ...
        assert_eq!(config.to_string(), "https://api.ethplorer.io/getAddressHistory/0x0");
    }

    #[test]
    fn get_address_transactions_config_works() {
        let config = get_address_transactions_config("", "0x0", &GetAddressTransactionsParams {
            limit: 50,
            timestamp: Default::default(),
        ...
        assert_eq!(config.to_string(), "https://api.ethplorer.io/getAddressTransactions/0x0");
    }
    ...
    fn get_top_config_works() {
        let config = get_top_config("", &GetTopParams {
            limit: 50,
    ...
    fn get_token_daily_price_history_config_works() {
        ...
        assert_eq!(config.network, NETWORK);
        assert_eq!(config.routes, vec![GET_TOKEN_PRICE_HISTORY_GROUPED_ROUTE, "0x0"]);
        assert_eq!(config.params, vec![
            ("apiKey".to_string(), "freekey".to_string()),
            ("period".to_string(), "50".to_string()),
        ]);
        assert_eq!(config.to_string(), "https://api.ethplorer.io/getTokenPriceHistoryGrouped/0x0");
    }
}
